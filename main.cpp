...
#else
    flags = 1;
    return ioctl(arg, FIONBIO, &flags);
#endif

}

inline void addFD(int epollfd, int fd, bool arg) {
...
inline void resetOneShot(int &epfd, int &fd) {
    struct epoll_event event;
    event.data.fd = fd;
...
std::string parseRequest(const std::string &for_parse) {
...
inline void f(int &fd, const std::string &request) {
    std::string f_name = parseRequest(request);
    if (f_name == "") {
